...
@implementation CJKTMaskGuideView
...
- (void)showInView:(UIView *)view {
    ...
    switch (self.coverType) {
        ...
        case CJKTMaskGuideViewCoverTypeColored:
            self.bgImg = [self.class imageWithColor:self.coverColor];
            break;
            
        case CJKTMaskGuideViewCoverTypeBlurred:
...
#pragma mark - init
- (instancetype)initWithBgColor:(UIColor *)bgColor revealView:(UIView *)revealView revealType:(CJKTMaskGuideViewRevealType)revealType layoutType:(CJKTMaskGuideViewLayoutType)layoutType {
    if (self = [super initWithFrame:[[UIScreen mainScreen]bounds]]) {
        
        [self commonSetupWithCoverType:CJKTMaskGuideViewCoverTypeColored revealType:revealType revealView:revealView layoutType:layoutType];
...
- (instancetype)initWithBlurRadius:(CGFloat)blurRadius revealView:(UIView *)revealView revealType:(CJKTMaskGuideViewRevealType)revealType layoutType:(CJKTMaskGuideViewLayoutType)layoutType {
    if (self = [super initWithFrame:[[UIScreen mainScreen]bounds]]) {
        
        [self commonSetupWithCoverType:CJKTMaskGuideViewCoverTypeBlurred revealType:revealType revealView:revealView layoutType:layoutType];
...
- (instancetype)initWithRevalView:(UIView *)revalView layoutType:(CJKTMaskGuideViewLayoutType)layoutType {
    if (self = [super initWithFrame:[[UIScreen mainScreen]bounds]]) {
        ...
        [self commonSetupWithCoverType:CJKTMaskGuideViewCoverTypeColored revealType:CJKTMaskGuideViewRevealTypeRect revealView:revalView layoutType:layoutType];
...
- (void)commonSetupWithCoverType:(CJKTMaskGuideViewCoverType)coverType revealType:(CJKTMaskGuideViewRevealType)revealType revealView:(UIView *)revealView layoutType:(CJKTMaskGuideViewLayoutType)layoutType {
...
- (void)setupAttachUI {
    ...
    neverBtn.titleLabel.font = [UIFont systemFontOfSize:15];
...
- (void)layoutSubviews {
    ...
    //neverBtn
    CGFloat neverBtnW = 100;
    CGFloat neverBtnH = 30;
    ...
    NSString *bundlePath = [[NSBundle mainBundle] pathForResource:@"CJKTMaskGuideCoverView" ofType:@"bundle"];
 
    //others
    switch (self.layoutType) {
        case CJKTMaskGuideViewLayoutTypeUP: {
        ...
        case CJKTMaskGuideViewLayoutTypeLeftUP: {
        ...
        case CJKTMaskGuideViewLayoutTypeLeft: {
        ...
        case CJKTMaskGuideViewLayoutTypeLeftDown: {
        ...
        case CJKTMaskGuideViewLayoutTypeDown: {
        ...
        case CJKTMaskGuideViewLayoutTypeRightDown: {
        ...
        case CJKTMaskGuideViewLayoutTypeRight: {
        ...
        case CJKTMaskGuideViewLayoutTypeRightUP: {
...
- (void)neverBtnClicked:(UIButton *)btn {
    ...
    [self dismiss];
    if (self.delegate && [self.delegate respondsToSelector:@selector(didClickedNeverBtn:)]) {
        [self.delegate didClickedNeverBtn:self];
    }
}
- (void)dismissBtnClicked:(UIButton *)btn {
    
    [self dismiss];
    if (self.delegate && [self.delegate respondsToSelector:@selector(didClickedDismissBtn:)]) {
        [self.delegate didClickedDismissBtn:self];
    }
}
